...
sizeLinks = (links, animate) ->
	...
	links.each ->
		if $(@).hasClass("settings_inactive")
			console.log("SETTINGS:" + $(@).attr("id"))
			return false
		if !$(@).hasClass("zoom-" + $("#geo_zoom_slider").slider("value"))
			console.log("SETTINGS:" + $(@).attr("id"))
			return false
		left_first = true
...
setUpControls = (zooms) ->
	...
	$("#active_groups_checkbox").prop("checked", true).change ->
		if $(@).is(":checked")
			for group in $(settings.groupsDOM)
				if group.hasClass("active") and !group.hasClass("zoom_inactive") and $("#" + group.attr("id"), "#map_container").length == 0
					$(".link.group" + group.attr("id").substring(group.attr("id").indexOf("-") + 1)).removeClass("settings_inactive").fadeIn(settings.ANIMATION_SPEED)
					placeGroupDOMOnMap group
					group.removeClass("settings_inactive")
		else
			$(".group.active").addClass("settings_inactive").detach().each ->
				$(".link.group" + $(@).attr("id").substring($(@).attr("id").indexOf("-") + 1)).addClass("settings_inactive").fadeOut(settings.ANIMATION_SPEED)
		fitGroups true, $(".group", "#map_container").length
		setTimeout (->
			sizeLinks $(".link.zoom-", "#map_container"), true)
			settings.ANIMATION_SPEED
	$("#inactive_groups_checkbox").prop("checked", true).change ->
		if $(@).is(":checked")
			for group in $(settings.groupsDOM)
				if group.hasClass("inactive") and !group.hasClass("zoom_inactive") and $("#" + group.attr("id"), "#map_container").length == 0
					$(".link.group" + group.attr("id").substring(group.attr("id").indexOf("-") + 1)).removeClass("settings_inactive").fadeIn(settings.ANIMATION_SPEED)
					placeGroupDOMOnMap group
					group.removeClass("settings_inactive")
		else
			$(".group.inactive").addClass("settings_inactive").detach().each ->
				$(".link.group" + $(@).attr("id").substring($(@).attr("id").indexOf("-") + 1)).addClass("settings_inactive").fadeOut(settings.ANIMATION_SPEED)
		fitGroups true, $(".group", "#map_container").length
		setTimeout (->
			sizeLinks $(".link", $("#map_container")), true)
			settings.ANIMATION_SPEED
